...
// getBinaryPath retorna la ruta del binario según el entorno
func getBinaryPath(component string) string {
	// Detectar si estamos en desarrollo o producción
	if isDevelopment() {
		// En desarrollo, usar 'go run'
		switch component {
		case "api":
			return "go"
		case "worker":
		...
		// En producción (Docker), verificar si existen binarios separados
		switch component {
		case "api":
			if _, err := os.Stat("./auth-api"); err == nil {
				return "./auth-api"
			}
			// Si no existe, usar el binario principal con argumentos
			return "./auth-service"
		case "worker":
...
// getBinaryArgs retorna los argumentos para el binario
func getBinaryArgs(component string) []string {
	if isDevelopment() {
		switch component {
		case "api":
			return []string{"run", "cmd/api/main.go"}
		case "worker":
			return []string{"run", "cmd/worker/main.go"}
		default:
			return []string{}
		}
	} else {
		// En producción, verificar si tenemos binarios separados
		switch component {
		case "api":
			if _, err := os.Stat("./auth-api"); err == nil {
				return []string{} // Sin argumentos para binario separado
			}
			// Si usamos el binario principal, pasar argumentos
			return []string{"-mode=api"}
		case "worker":
			if _, err := os.Stat("./auth-worker"); err == nil {
				return []string{} // Sin argumentos para binario separado
			}
			// Si usamos el binario principal, pasar argumentos
			return []string{"-mode=worker"}
		default:
			return []string{}
		}
	}
...
func isDevelopment() bool {
	...
	// Verificar si existe el directorio de código fuente
	if _, err := os.Stat("cmd"); err == nil {
		// También verificar si está disponible el comando 'go'
...
// isInDocker verifica si estamos ejecutándose en un contenedor Docker
func isInDocker() bool {
	...
	// Método 2: Verificar si el PID es 1 (típico en contenedores)
	if os.Getpid() == 1 {
		return true
	}
	
	// Método 3: Verificar variable de entorno común en contenedores
...
func (pm *ProcessManager) startAPI() error {
	...
	// Obtener ruta del binario y argumentos
	binaryPath := getBinaryPath("api")
	args := getBinaryArgs("api")

	if binaryPath == "" {
		return fmt.Errorf("no se pudo determinar el binario del API")
	}

	// Configurar comando del API
	pm.apiCmd = exec.Command(binaryPath, args...)
	pm.apiCmd.Stdout = os.Stdout
	pm.apiCmd.Stderr = os.Stderr
...
func (pm *ProcessManager) startWorker() error {
	...
	// Obtener ruta del binario y argumentos
	binaryPath := getBinaryPath("worker")
	args := getBinaryArgs("worker")

	if binaryPath == "" {
		return fmt.Errorf("no se pudo determinar el binario del Worker")
	}

	// Configurar comando del Worker
	pm.workerCmd = exec.Command(binaryPath, args...)
	pm.workerCmd.Stdout = os.Stdout
	pm.workerCmd.Stderr = os.Stderr
...
func logSystemInfo() {
	...
	// Mostrar si estamos en desarrollo o producción
	if isDevelopment() {
		log.Println("🔧 Entorno: Desarrollo")
	} else {
		log.Println("🚀 Entorno: Producción")
	}
}
